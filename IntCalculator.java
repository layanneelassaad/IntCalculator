...
public class IntCalculator {
    ...
    public String infixToPostfix() throws StackException {
        ...
        for (int i=0; i<infixExpression.length(); i++) {
            ...
            //Checking if the integer is one or more digits
            if (isDigit(c)) {
                postfix.append(c);
    ...
    public int evaluatePostfix()
        ...
        for (int i=0; i<postfixExpression.length(); i++) {
            ...
            if (isDigit(c)){
                ...
                int index = i;
                while (index+1<postfixExpression.length() && isDigit(postfixExpression.charAt(index+1))){
                    value += String.valueOf(postfixExpression.charAt(index+1));
                    index++;
                }
                i= index;
                ...
                int value = -1 * stack.pop();
                stack.push(value);

            } else if (isBinaryOperator(c)){
                int value1 = stack.pop();
                int value2 = stack.pop();
                switch (c) { // switch statement for every operator
                    case '+':
